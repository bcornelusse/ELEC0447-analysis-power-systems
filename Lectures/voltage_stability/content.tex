...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item It thus does not know whether it operates in a stable or unstable region.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item an incident on the network (e.g. a line tripping) may reduce the maximum power transferable below the target power $P_0$, leading to voltage instability and collapse.
    \end{itemize} 
\end{itemize}
...
Replacing $\delta$ by $\delta^{max}$ in the equation of $P_R$, we have:
...
This is a typical case of \textbf{Long-term Voltage Instability}.
...
\begin{columns}
    ...
    \begin{column}{0.4\textwidth}
        \begin{itemize}
            ...
            \item $C$ curves correspond to different electromechanical torques depending on the slip $s$, which is related to synchronous speed $\omega_s$ and actual rotation speed $\omega_r$ by $s=\frac{\omega_s - \omega_r}{\omega_s}$.
...
\begin{itemize}
    ...
    \item This is considered a \textbf{Short-term Voltage Instability} as this phenomenon is much quicker than what we have with OLTCs (it takes several seconds to change tap positions).
...
In transmission systems, we have synchronous machines. They can provide or consume reactive power.
\begin{center}
...
\begin{frame}[allowframebreaks]{Primary, secondary and tertiary voltage control}
    \begin{itemize}
        \item When a disturbance occurs, or following a change in load (cf. 'duck curve'), the \textit{primary} voltage control loops maintain suitable voltage levels close to the large power plants equipped with AVRs.
        \begin{itemize}
...
\begin{itemize}
    ...
    \item Before the advent of distributed energy resources (\emph{PV panels, batteries,...}), the power was flowing from the substation node, down to the end of the feeders.
...
\begin{frame}{Reverse power flows in distribution systems}
\begin{itemize}
    \item In distribution systems, $X/R < 1$. Active power flows have actually a greater influence on voltage magnitudes than reactive power flows.
...
\begin{itemize}
    ...
    \item Different voltage instabilities, caused by slow-acting devices like OLTCs (long-term instabilities), or by faster dynamics like stalling of induction motors (short-term instabilities).

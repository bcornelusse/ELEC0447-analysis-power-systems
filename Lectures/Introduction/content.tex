...
\item Gas power plants include small CHP (Combined Heat Power) units
...
\begin{itemize}
\item Early retirement of gas power plants not competitive enough on electricity market, too expensive to maintain
\begin{itemize}
...
\item $\text{Capacity Factor} = (3.408 \times 10^6) / (1186 \times 365 \times 24) = 32\%$
\end{itemize}
...
\item a deficit (resp. excess) of generation wrt load results in a decrease (resp. increase) of rotation speed (and hence, frequency)
...
\begin{itemize}
\item DC more attractive than AC for distances $\ge$ 50 km : owing to capacitive effects of AC cables
...
\end{itemize}
\item two networks that have the same nominal frequency but cannot be merged into a single AC network, e.g. for stability reasons
\begin{itemize}

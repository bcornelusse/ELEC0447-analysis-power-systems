...
\begin{frame}[allowframebreaks]{Voltages and currents}
    ...
    $$
    \frac{\bar{V}_{an}-\bar{V}_{nN'}}{R_a} + \frac{\bar{V}_{bn}-\bar{V}_{nN'}}{R_b} + \frac{\bar{V}_{cn}-\bar{V}_{nN'}}{R_c} = 0    
    $$
...
\begin{itemize}
    ...
    \item Therefore, voltage control is done locally to avoid transferring reactive power over long distances. We will come back to this point in the "voltage stability" lecture.
\end{itemize}
\end{frame}
...
\begin{enumerate}
    \item Choose 3\footnote{When there are transformers, one base voltage per voltage level is preferable} "fundamental" base values:
    \begin{itemize}
...
Data in per unit is by definition dimensionless; pay attention to base value units w.r.t. physical value units!
\end{frame}
...
\begin{itemize}
    \item We can also apply the \textbf{symmetrical component transformation to network components}: loads, transmission lines, transformers, machines. 
    \item This yields positive-, negative-, and zero-sequence networks
    ...
    \item We can also \textbf{design protections} based on the value of the symmetrical component values! For instance, zero-sequence current is a direct indicator of a ground fault, and its detection is a standard practice for overcurrent ground fault protection.
\end{itemize}
\end{frame}
...
The $dq0$ transformation converts three-phase ($abc$) quantities into a new $dq0$ frame.

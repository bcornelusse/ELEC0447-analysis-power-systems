...
\begin{frame}[allowframebreaks]{Reminder: generator model and step up transformer}
    ...
    \end{figure}
\begin{itemize}
    \item Consider a generator bus connected through a transformer to an infinite bus system ($\bar{V}_B = V_B$ is an ideal voltage source).
    ...
    \item Finally, $X_{tr}$ is the leakage reactance of the transformer.
\end{itemize}
\end{frame}
...
Let us define the following quantities: $J_m$ is the moment-of-inertia of the rotational system, $T_m$ is the mechanical torque, $T_e$ is the electrical torque, $\omega_m$ is the rotor speed and $\delta_m$ the rotor angle (in mechanical radians). Using Newton's Second Law, one has (we omit the damping torque for now, we will discuss its impact later.):
...
One can write the swing equation by substituting $J_m$ with $H_{gen}$:
...
Assume $\delta_0$ is an equilibrium point and an incident occurs. For the system to stabilize, it must exist an angle $\delta_{m}$ at which the kinetic energy becomes 0, and thus:
$$\int_{\delta_0}^{\delta_m} \left(P_{m,pu}-P_{e,pu}\right) d\theta = 0$$    
\end{block}

\textbf{Demonstration:}
The swing equation established in (\ref{eq:swing}) is
$$\left(\frac{2H}{\omega_{syn}}\right) \frac{d^2\delta}{dt^2} = P_{m,pu} - P_{e,pu}$$
Let's rearrange the terms and multiply both sides by $d\delta/dt$:
...
Since $\delta_0$ is an equilibrium point, \emph{i.e.} $d\delta/dt|_{\delta = \delta_0} = 0$, we have:
...
$$\underbrace{\int_{\delta_0}^{\mathcolor{green}{\delta_{cl}}} \left(P_{m,pu}-\mathcolor{blue}{P_{e,fault,pu}}\right) d\theta}_{\text{Area A}} - \underbrace{\int_{\mathcolor{green}{\delta_{cl}}}^{\delta_{m}} \left(\mathcolor{red}{P_{e,post-fault,pu}}-P_{m,pu}\right) d\theta}_{\text{Area B}} = 0$$
\begin{columns}
    \begin{column}{0.55\textwidth}
        \begin{itemize}
...
\begin{frame}{Main types of DSA analysis}

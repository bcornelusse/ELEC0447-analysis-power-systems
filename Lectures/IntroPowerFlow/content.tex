...
\begin{frame}[allowframebreaks]{Newton-Raphson for the power flow problem}
    ...
    \begin{itemize}
        ...
        \item $\mathbf{\bar{V}}_x$ is the subvector of $\mathbf{\bar{V}}$ that gathers the unknown voltage magnitudes and angles at the corresponding buses
...
\begin{frame}[allowframebreaks]{Fast decoupled power flow}
    ...
    If we apply these ideas strictly, we can subdivide the problem in two much simpler subproblems:
    \begin{itemize}
    ...
    This procedure, through the sub-Jacobians that are computed, also provides information useful for \textit{sensitivity analysis}.
\end{frame}
...
\begin{frame}[allowframebreaks]{DC power flow}
    ...
    This is useful for fast simulations, or when including a power flow model in an optimization problem, e.g. \href{https://bcornelusse.github.io/material/CoursEM20170331.pdf}{day-ahead market coupling}.
\end{frame}
